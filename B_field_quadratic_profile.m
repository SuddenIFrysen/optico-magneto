function B = B_field_quadratic_profile(target_domain, Str, a, b, Rc)
%B_field_quadratic
...
%       The B-field over the graphene is given as B(r) = Str*(1+a*(r/Rc)^2+b*(r/R*scale)^6)

r_rel = @(X,Y,Z) sqrt((X-target_domain.Origin(1)).^2 + (Y-target_domain.Origin(2)).^2 + (Z-target_domain.Origin(3)).^2 );
B = @(X,Y,Z) Str * permute(target_domain.Normal, [1 3 2]) .* (1 + a*(r_rel(X, Y, Z) / Rc).^2 + b*(r_rel(X, Y, Z) / Rc).^6);
end